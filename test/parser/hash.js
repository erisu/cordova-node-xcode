...
describe('parser/hash', () => {
    it('should have the top-line comment in place', () => {
        assert.strictEqual(rawProj.headComment, '!$*UTF8*$!');
    });

    it('should parse a numeric attribute', () => {
        assert.strictEqual(project.archiveVersion, 1);
        assert.strictEqual(project.objectVersion, 45);
    });
    ...
    it('should parse non-commented hash things', () => {
        assert.strictEqual(project.nonObject, '29B97313FDCFA39411CA2CEF');
    });
});
