...
PBXProject.prototype.removeBuildProperty = function (prop, build_name) {
    ...
    for (const key in configurations) {
        configuration = configurations[key];
        if (
            configuration.buildSettings[prop] &&
...
PBXProject.prototype.addTarget = function (name, type, subfolder, bundleId) {
    ...
    if (targetType === 'app_extension') {
        ...
        const watch2Target = this.getTarget(producttypeForTargettype('watch2_app'));
        if (watch2Target) {
            this.addBuildPhase(
                [targetName + '.appex'],
                'PBXCopyFilesBuildPhase',
...
function correctForPath (file, project, group) {
    ...
    if (project.pbxGroupByName(group) && project.pbxGroupByName(group).path) { file.path = file.path.replace(r_group_dir, ''); }
